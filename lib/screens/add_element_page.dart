...
class _AddElementPageState extends State<AddElementPage> {
  ...
  Future<void> _uploadAndSave() async {
    ...
    try {
      ...
      if (_bytes == null && widget.existingElement != null) {
        ...
        storagePath = widget.existingElement!['imageStoragePath'] as String? ?? '';
      } else {
        ...
        final up = await _storage.uploadTempImage(bytes: _bytes!, filename: _filename!);
      ...
      final frameRef = FirebaseFirestore.instance.collection('comic_game').doc('frames').collection('frames').doc(widget.frameId);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.existingElement != null ? 'Edit Element' : 'Add Element')),
      body: Padding(
        ...
        child: Column(
          children: [
            if (widget.frameImageUrl.isNotEmpty)
              Expanded(
                child: LayoutBuilder(builder: (context, constraints) {
                  ...
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    ...
                    if (ctx != null) {
                      ...
                      if (size != null) {
                        ...
                        if (newW != _imageW || newH != _imageH || newLeft != _imageLeft || newTop != _imageTop) {
                          setState(() {
                            _imageW = newW;
                            _imageH = newH;
                            _imageLeft = newLeft;
                  ...
                  final base = (_imageW > 0 && _imageH > 0) ? (_imageW < _imageH ? _imageW : _imageH) : (w < h ? w : h);
                  ...
                  final maxCenterX = (imgW - elemW / 2.0).clamp(minCenterX, imgW);
                  var centerX = (_x * imgW).clamp(minCenterX, maxCenterX);

                  final minCenterY = elemH / 2.0;
                  final maxCenterY = (imgH - elemH / 2.0).clamp(minCenterY, imgH);
                  ...
                  if ((newX - _x).abs() > 0.0001 || (newY - _y).abs() > 0.0001) {
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      if (mounted) setState(() {
                        _x = newX;
                        _y = newY;
                  ...
                  return Stack(
                    ...
                    children: [
                      ...
                      Positioned.fill(
                        child: Center(child: Image.network(widget.frameImageUrl, key: _imageKey, fit: BoxFit.contain)),
                      ),
                      if (_uploadedImageUrl != null)
                        Positioned(left: left, top: top, width: elemW, height: elemH, child: Image.network(_uploadedImageUrl!, fit: BoxFit.contain)),
                      if (_uploadedImageUrl == null && _bytes != null)
                        Positioned(left: left, top: top, width: elemW, height: elemH, child: Image.memory(_bytes!, fit: BoxFit.contain)),
                    ],
                  );
                }),
              )
            ...
            Row(children: [
              ElevatedButton.icon(onPressed: _pickElementImage, icon: const Icon(Icons.image), label: const Text('Pick image')),
              const SizedBox(width: 12),
              Expanded(child: Text(_filename ?? (_uploadedImageUrl != null ? 'Using existing image' : 'No element picked'))),
            ]),

            const SizedBox(height: 12),

            Column(children: [
              Row(children: [const Text('X:'), Expanded(child: Slider(value: _x, onChanged: (v) => setState(() => _x = v), min: 0, max: 1))]),
              Row(children: [const Text('Y:'), Expanded(child: Slider(value: _y, onChanged: (v) => setState(() => _y = v), min: 0, max: 1))]),
              Row(children: [const Text('Scale:'), Expanded(child: Slider(value: _scale, onChanged: (v) => setState(() => _scale = v), min: 0.02, max: 1.0))]),
            ]),

            const SizedBox(height: 12),
            if (_busy) const CircularProgressIndicator(),
            Row(children: [
              Expanded(child: ElevatedButton(onPressed: _busy ? null : _uploadAndSave, child: Text(widget.existingElement != null ? 'Save Element' : 'Add Element'))),
            ])
          ],
        ),
      ),
