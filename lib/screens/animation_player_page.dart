...
class AnimationPlayerPage extends StatefulWidget {
  ...
  const AnimationPlayerPage({super.key, required this.frameImageUrl, required this.element});

  @override
  State<AnimationPlayerPage> createState() => _AnimationPlayerPageState();
}

class _AnimationPlayerPageState extends State<AnimationPlayerPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (raw is List) {
      _animations = raw.map((e) => Map<String, dynamic>.from(e as Map)).toList();
    }
  ...
  void _playNext() {
    ...
    final dur = (a['duration'] is num) ? (a['duration'] as num).toDouble() : ((a['seconds'] is num) ? (a['seconds'] as num).toDouble() : 1.0);
    final matRaw = a['matrix'];
    // translation values are stored as fractions of the displayed frame width/height
    final txFrac = (a['translateX'] is num) ? (a['translateX'] as num).toDouble() : 0.0;
    final tyFrac = (a['translateY'] is num) ? (a['translateY'] as num).toDouble() : 0.0;
    ...
    if (matRaw is List && matRaw.length == 16) {
      ...
      _index++;
      if (_index < _animations.length) Future.delayed(const Duration(milliseconds: 100), _playNext);
    }
  }
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          SizedBox(
            ...
            child: LayoutBuilder(builder: (context, constraints) {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                ...
                if (ctx != null) {
                  ...
                  if (size != null) {
                    ...
                    if (newW != _frameW || newH != _frameH || newLeft != _frameLeft || newTop != _frameTop) {
                      setState(() {
                        _frameW = newW;
                        _frameH = newH;
                        _frameLeft = newLeft;
              ...
              final pos = widget.element['position'] as Map<String, dynamic>? ?? {'x': 0.5, 'y': 0.5};
              final nx = (pos['x'] is num) ? (pos['x'] as num).toDouble() : 0.5;
              final ny = (pos['y'] is num) ? (pos['y'] as num).toDouble() : 0.5;
              final elemScale = (widget.element['scale'] is num) ? (widget.element['scale'] as num).toDouble() : 0.2;

              final base = (_frameW > 0 && _frameH > 0) ? (_frameW < _frameH ? _frameW : _frameH) : (w < h ? w : h);
              ...
              return Stack(children: [
                ...
                Positioned.fill(child: Center(child: Image.network(widget.frameImageUrl, key: _frameKey, fit: BoxFit.contain))),
                // transformed element positioned using saved normalized position and scale
                Positioned(left: left, top: top, width: elemW, height: elemH, child: Transform(transform: _current, alignment: Alignment.center, child: Image.network(widget.element['imageUrl'] as String? ?? '', width: elemW, height: elemH, fit: BoxFit.contain))),
              ]);
            }),
          ),
          const SizedBox(height: 12),
          Text('Playing ${_animations.length} animations, index: ${_index + 1}/${_animations.length}'),
        ]),
      ),
    );
  }
